	...
	public function get_shipping_dimensions( $tag_name = 'shipping_dimensions', $result_xml = '' ) {
		...
		if ( empty( $shipping_length_source ) || $shipping_length_source === 'woo_shippings' ) {
			if ( $this->get_product()->has_dimensions() ) {
				$shipping_length_value = $this->get_offer()->get_length();
			...
			$tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $shipping_length_source ) );
			if ( empty( $tag_value ) ) {
		...
		if ( empty( $shipping_width_source ) || $shipping_width_source === 'woo_shippings' ) {
			if ( $this->get_product()->has_dimensions() ) {
				$shipping_width_value = $this->get_offer()->get_width();
			...
			$tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $shipping_width_source ) );
			if ( empty( $tag_value ) ) {
		...
		if ( empty( $shipping_height_source ) || $shipping_height_source === 'woo_shippings' ) {
			if ( $this->get_product()->has_dimensions() ) {
				$shipping_height_value = $this->get_offer()->get_height();
			...
			$tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $shipping_height_source ) );
			if ( empty( $tag_value ) ) {
		...
		if ( empty( $shipping_weight_source ) || $shipping_weight_source === 'woo_shippings' ) {
			$shipping_weight_value = $this->get_offer()->get_weight();
			...
			$tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $shipping_weight_source ) );
			if ( empty( $tag_value ) ) {
