...
<div id="xfgmc_wrap" class="wrap">
	...
	<h1>XML for Google Merchant Center - <?php esc_html_e( 'Feeds list', 'xml-for-google-merchatnt-center' ); ?></h1>
	<form action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" method="post" enctype="multipart/form-data">
		<?php wp_nonce_field( 'xfgmc_nonce_action_add_new_feed', 'xfgmc_nonce_field_add_new_feed' ); ?>
		<input value="+ <?php esc_html_e( 'Add New Feed', 'xml-for-google-merchatnt-center' ); ?>" class="button-primary"
	...
	<?php printf( '<p style="text-align: right;"><strong>%s:</strong> %s, <strong>%s:</strong> %s</p>',
		esc_html__( 'Current site time', 'xml-for-google-merchatnt-center' ),
		current_time( 'mysql', 0 ),
		esc_html__( 'Current server time', 'xml-for-google-merchatnt-center' ),
